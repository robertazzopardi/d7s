...
impl TableNavigationHandler {
    ...
    pub fn clamp_selection<T: TableData + Clone>(table: &mut DataTable<T>) {
    ...
    pub fn clamp_columns<T: TableData + Clone>(table: &mut DataTable<T>) {
        ...
        if let Some(table_data) = table_data {
            match key {
                KeyCode::Char('j') | KeyCode::Down => {
                    ...
                    Self::clamp_selection(table_data);
                }
        ...
        if let Some(table_widget) = &mut sql_executor.table_widget {
            match key {
                KeyCode::Char('j') | KeyCode::Down => {
                    ...
                    Self::clamp_selection(table_widget);
                }
                KeyCode::Char('k') | KeyCode::Up => {
                ...
                KeyCode::Char('h' | 'b') | KeyCode::Left => {
                    ...
                    Self::clamp_columns(table_widget);
                    // Adjust offset to ensure selected column is visible
                    if let Some(selected_col) =
